...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=2cm,auto,>=latex']
        \node[block,dashed border,align=center] at (2,4) (n1) {$\Sys_{NL}$};
        \node[block,double border,align=center] at (2,2) (n2) {$f_{NL}$};
        \node[block,dashed border,align=center] at (2,0) (n3) {model of\\ $\Sys_{NL}$};
        \node[sum] at (4,2) (sum) {};
        \node[left] at (0,4) (u) {$u(t)$};
...
\begin{remark}[Block scheme notation for non-linear system] 
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=2cm,auto,>=latex']
        \node[block,dashed border,align=center] at (0,0) (n) {non-linear\\dyn. T.I. sys.};
...
\paragraph{Architecture \#1} Use a \emph{Dynamical Recurrent Neural Network (Dynamic RNN)} in which we update \emph{static neurons} into \emph{dynamic neurons}.
...
\begin{remark}[Pros \& Cons]
	The advantage is that usually in IIR architecture $n_u$ and $n_y$ are much smaller (thanks to recursion): lower computation 
	effort. 

	For the disadvantages we have to notice that only for the training part we use $x(t)$ data from a physical sensor; then, in production, the recursion comes to play since we need to plug-in the output $\hat{x}(t|t)$ in the recursive part: this feedback can provide instability. 
    ...
    \begin{tikzpicture}[node distance=2cm,auto,>=latex']
        ...
        \draw[->] (xn) -- (xn-|sys.west) node[pos=0.5] {$\hat{x}(t-n_u|t-n_u)$};
...
\begin{example}[Example \ref{ex:KF_full-proc} continued]
