...
{\huge TODO DRE, A.R.E acron. gls... + rivedere}
...
\begin{definition}[Kalman Filter in Filter Form] \label{KF-Filter_Form_sol}
    ...
    \end{align*}
    while the DRE equation (\ref{eq:KF-DRE}) remains unchanged.\\
...
\begin{obs}
    Gain of K.F. in prediction form (eq. \ref{eq:KF-gain} assuming $V_{12}=0$):
    \[
...
The fact that K.F. is an LTV system is the source of 2 problems:
\begin{itemize}
    \item Checking the asymptotic stability of K.F. algorithm is very difficult, since the stability check of an LTV system is not simple as the stability check for LTI.
    \item Computational problem: $K(t)$ must be computed at each sampling time (e.g. every 5ms), including the inversion of $HP(t)H\transpose+V_2$ ($p\times p$ matrix) and the computation of $P(t)$ using the DRE.
\end{itemize}
...
\\
Using $\bar{K}$ the K.F. becomes an LTI system.

Let's analyze the asymptotic stability of the State equation (eq. \ref{eq:KF-state}) of the asymptotic K.F. when $\bar{K}$ is used (assuming it exists).
...
\paragraph{Condition for Asymptotic Stability of KF}

 If $\bar{K}$ exists, the K.F. is asymptotically stable if and only if all the eigenvalues of $F-\bar{K}H$ are strictly inside the unit circle.

\begin{obs}
    The stability of the system $\Sys$ is related to matrix $F$, whereas the stability of K.F. is related to matrix $F-\bar{K}H$.

    Therefore, K.F. can be asymptotically stable even if the system is unstable.
\end{obs}
...
In order to check that, we need to check the converge properties of D.R.E.
...
\begin{recall}[Stability of a dynamical autonomous system]
    ...
    \end{center}
\end{recall}
D.R.E. is an autonomous discrete time system, thus we impose $\bar{P} = f(\bar{P})$, where $f(\bar{P})$ is the DRE (eq. \ref{eq:KF-DRE}) evaluated in $\bar{P}$:

\begin{flalign}\label{eq:KF-ARE}
    \nameeq{\bar{P} = \left( F\bar{P}F\transpose + V_1 \right)-\left(F\bar{P}H\transpose + V_{12}\right)\left(H\bar{P}H\transpose + V_2\right)^{-1}\left(F\bar{P}H\transpose+V_{12}\right)\transpose}{ARE}
\end{flalign}

\begin{definition}[Algebraic Riccati Equation (ARE)]
    The equation \ref{eq:KF-ARE} is a non-linear, matrix, static algebraic equation, known as \emph{Algebraic Riccati Equation (ARE)}.
\end{definition}

If a steady state $\bar{P}$ solution of D.R.E. (eq. \ref{eq:KF-DRE}) does exists, it must be a solution of A.R.E (eq. \ref{eq:KF-ARE}).
There remains 3 questions:
\begin{enumerate}
    \item \textbf{Existence}: does A.R.E. have a semi-definite positive solution?
    \item \textbf{Convergence}: if exists, does the D.R.E. converges to $\bar{P}$?
    \item \textbf{Stability}: is the corresponding $\bar{K}$ such that the K.F. is asymptotically stable?
\end{enumerate}
...
\begin{theorem}[First Asymptotic KF Theorem]\label{th:1KF_as}
    Assumptions: $V_{12} = 0$ and the system is asymptotically stable (i.e. all eigenvalues of $F$ are strictly inside the unit circle).
    Then:
    \begin{enumerate}
        \item A.R.E. has one and only one semi-definite positive solution: $\bar{P} \ge 0$.
        \item D.R.E. converges to $\bar{P}$, $\forall P_0 \ge 0$ ($P_0$: initial semi-definite positive condition).
        \item The corresponding $\bar{K}$ is s.t. the K.F. is asymptotically stable.
    \end{enumerate}
\end{theorem}

\begin{recall}[Observability and Controllability]
    Recall on Observability and Controllability needed for the introduction of the Second Asymptotic KF Theorem (\ref{th:2KF_as}). 
...
\begin{theorem}[Second Asymptotic KF Theorem]\label{th:2KF_as}
    Assumptions: $V_{12} = 0$, $(F, H)$ is observable and $(F, \Gamma)$ is controllable.
    Then:
    \begin{enumerate}
        \item A.R.E. has one and only one definite positive solution $\bar{P} > 0$.
        \item D.R.E. converges to $\bar{P}$, $\forall P_0 \ge 0$ ($P_0$: initial semi-definite positive condition).
        \item The corresponding $\bar{K}$ is such that the K.F. is asymptotically stable.
    \end{enumerate}
\end{theorem}
...
These two theorems are very useful in practice because we can fully avoid the (very difficult) direct convergence analysis of D.R.E.
